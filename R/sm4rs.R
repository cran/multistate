...
sm4rs <-function(t1, t2, sequence, weights=NULL, dist, cuts.12=NULL, cuts.13=NULL,cuts.14=NULL,cuts.23=NULL,cuts.24=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.14=NULL, ini.dist.23=NULL, ini.dist.24=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.14=NULL, init.cov.14=NULL, names.14=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, cov.24=NULL, init.cov.24=NULL, names.24=NULL, p.age, p.sex, p.year, p.rate.table, conf.int=TRUE, silent=TRUE, precision=10^(-6))
...
if (is.na(min(p.age)))
		cat("Warning: individuals with missing values for 'p.age' will be removed \n")
...
c12<-function(t1, t2, z12, z13, z14, z23, z24, cut12, cut13, cut14, cut23, cut24, H4P.t2)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14) - H23(t2, z23, cut23) - H24(t2, z24, cut24)- H4P.t2)}
...
c123<-function(t1, t2, z12, z13, z14, z23, z24, cut12, cut13, cut14, cut23, cut24, H4P.t2)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14) + log.h23(t2, z23, cut23) - H23(t2, z23, cut23) - H24(t2, z24, cut24) - H4P.t2)}

c124<-function(t1, t2, z12, z13, z14, z23, z24, cut12, cut13, cut14, cut23, cut24, H4P.t2, h4P.t2)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14) + log(exp(log.h24(t2, z24, cut24))+ h4P.t2) - H23(t2, z23, cut23) - H24(t2, z24, cut24) - H4P.t2)}
...
#age.year<-age
  ...
  while (j<trunc(t.year[i])) {
    ...
    #sumcum[i]<-sumcum[i]+h4P(rate.table,age[i],sex[i],year[i],j)
    j<-j+1
    } #end of time loop
   #sumcum[i]<-sumcum[i]+(t.year[i]-trunc(t.year[i]))*365.24*h4P(rate.table,age.year[i]*365.24,sex[i],year[i],j*365.24)
...
.c12.H4P.t2<-H4P(rate.table=p.rate.table,age=.c12.p.age,sex=.c12.p.sex,year=.c12.p.year,.d12.1+.d12.2)
...
.c123.H4P.t2<-H4P(rate.table=p.rate.table,age=.c123.p.age,sex=.c123.p.sex,year=.c123.p.year,.d123.1+.d123.2)
...
.c124.H4P.t2<-H4P(rate.table=p.rate.table,age=.c124.p.age,sex=.c124.p.sex,year=.c124.p.year,.d124.1+.d124.2)                                            
.c124.h4P.t2<-h4P(rate.table=p.rate.table,age=.c124.p.age,sex=.c124.p.sex,year=.c124.p.year,.d124.1+.d124.2)
