...
m4rs <- function(t1, t2, sequence, weights=NULL, dist, cuts.12=NULL,cuts.13=NULL,cuts.14=NULL,cuts.23=NULL,cuts.24=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.14=NULL, ini.dist.23=NULL, ini.dist.24=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.14=NULL, init.cov.14=NULL, names.14=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, cov.24=NULL, init.cov.24=NULL, names.24=NULL, p.age, p.sex, p.year, p.rate.table, conf.int=TRUE, silent=TRUE, precision=10^(-6))
...
if (is.na(min(t1)))
		cat("Warning: individuals with missing values for 't1' are deleted \n")
...
if (!all(is.na(t2[which(sequence==1 | sequence==13 | sequence==14)])))		
		stop("Arguments 't2' should be NA for right-censored individuals in X=1 or individuals who directly transited from X=1 to X=3 or from X=1 to X=4")
if (min(t2-t1,na.rm=T)<=0)
		stop("Arugment 't2' have to higher than 't1'")
...
{		
	...
	if (is.na(min(weights)))
		cat("Warning: individuals with missing values for 'weights' will be removed from the analysis \n")
}
...
if (sum(apply(sapply(data.frame(cov.14),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.14),is.na),1,sum)),"individuals with missing values on 'cov.14' will be removed from the analysis \n")
...
if (sum(apply(sapply(data.frame(cov.24),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.24),is.na),1,sum)),"individuals with missing values on 'cov.24' will be removed from the analysis \n")
...
if (is.na(min(p.age)))
		cat("Warning: individuls with missing values for 'p.age' will be removed from the analysis \n")
...
if (is.na(min(p.sex)))
		cat("Warning: individuls with missing values for 'p.sex' will be removed from the analysis \n")
...
if (is.na(min(p.year)))
		cat("Warning: individuls with missing values for 'p.year' will be removed from the analysis \n")
...
c1<-function(t1, z12, z13, z14, cut12, cut13, cut14, H4P.t1)
 {return( -H12(t1, z12, cut12) - H13(t1, z13, cut13)- H14(t1, z14, cut14)- H4P.t1)}

c12<-function(t1, t2, z12, z13, z14, z23, z24, cut12, cut13, cut14, cut23, cut24, H4P.t2)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14) - H23(t1, t2, z23, cut23) - H24(t1, t2, z24, cut24)- H4P.t2)}

c13<-function(t1, z12, z13, z14, cut12, cut13, cut14, H4P.t1)
 {return(log.h13(t1, z13, cut13) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14)- H4P.t1)}

c14<-function(t1, z12, z13, z14, cut12, cut13, cut14, H4P.t1, h4P.t1)
 {return(log(exp(log.h14(t1, z14, cut14))+ h4P.t1) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14)- H4P.t1)}

c123<-function(t1, t2, z12, z13, z14, z23, z24, cut12, cut13, cut14, cut23, cut24, H4P.t2)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14) + log.h23(t2, z23, cut23) - H23(t1, t2, z23, cut23) - H24(t1, t2, z24, cut24) - H4P.t2)}

c124<-function(t1, t2, z12, z13, z14, z23, z24, cut12, cut13, cut14, cut23, cut24, H4P.t2, h4P.t2)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H14(t1, z14, cut14) + log(exp(log.h24(t2, z24, cut24))+ h4P.t2) - H23(t1, t2, z23, cut23) - H24(t1, t2, z24, cut24) - H4P.t2)}
...
.c1.H4P.t1<-H4P(rate.table=p.rate.table,age=.c1.p.age,sex=.c1.p.sex,year=.c1.p.year,.d1.1)
...
.c12.H4P.t2<-H4P(rate.table=p.rate.table,age=.c12.p.age,sex=.c12.p.sex,year=.c12.p.year,.d12.2)
...
.c13.14 <- as.matrix(cov.14.mat[(sequence==13 & .na),])
.c13.p.age <- p.age[(sequence==13 & .na)]
.c13.p.sex <- p.sex[(sequence==13 & .na)]
.c13.p.year <- p.year[(sequence==13 & .na)]
.c13.H4P.t1<-H4P(rate.table=p.rate.table,age=.c13.p.age,sex=.c13.p.sex,year=.c13.p.year,.d13.1)
...
.c14.p.age <- p.age[(sequence==14 & .na)]
.c14.p.sex <- p.sex[(sequence==14 & .na)]
.c14.p.year <- p.year[(sequence==14 & .na)]
.c14.H4P.t1<-H4P(rate.table=p.rate.table,age=.c14.p.age,sex=.c14.p.sex,year=.c14.p.year,.d14.1)
.c14.h4P.t1<-h4P(rate.table=p.rate.table,age=.c14.p.age,sex=.c14.p.sex,year=.c14.p.year,.d14.1)
...
.c123.H4P.t2<-H4P(rate.table=p.rate.table,age=.c123.p.age,sex=.c123.p.sex,year=.c123.p.year,.d123.2)
...
.c124.p.age <- p.age[(sequence==124 & .na)]
.c124.p.sex <- p.sex[(sequence==124 & .na)]
.c124.p.year <- p.year[(sequence==124 & .na)]
.c124.H4P.t2<-H4P(rate.table=p.rate.table,age=.c124.p.age,sex=.c124.p.sex,year=.c124.p.year,.d124.2)                                            
.c124.h4P.t2<-h4P(rate.table=p.rate.table,age=.c124.p.age,sex=.c124.p.sex,year=.c124.p.year,.d124.2)
...
return( -1*(
 sum( .w1 * c1(.d1.1, .c1.12, .c1.13, .c1.14, cuts.12, cuts.13, cuts.14, .c1.H4P.t1) ) +
 sum( .w12 * c12(.d12.1, .d12.2, .c12.12, .c12.13, .c12.14, .c12.23, .c12.24, cuts.12, cuts.13, cuts.14, cuts.23, cuts.24, .c12.H4P.t2) ) +
 sum( .w13 * c13(.d13.1, .c13.12, .c13.13, .c13.14, cuts.12, cuts.13, cuts.14, .c13.H4P.t1) ) +
 sum( .w14 * c14(.d14.1, .c14.12, .c14.13, .c14.14, cuts.12, cuts.13, cuts.14, .c14.H4P.t1, .c14.h4P.t1) ) +
 sum( .w123 * c123(.d123.1, .d123.2, .c123.12, .c123.13, .c123.14, .c123.23, .c123.24, cuts.12, cuts.13, cuts.14, cuts.23, cuts.24, .c123.H4P.t2) ) +
 sum( .w124 * c124(.d124.1, .d124.2, .c124.12, .c124.13, .c124.14, .c124.23, .c124.24, cuts.12, cuts.13, cuts.14, cuts.23, cuts.24, .c124.H4P.t2, .c124.h4P.t2) ) ) )
