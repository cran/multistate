...
m3 <- function(t1, t2, sequence, weights=NULL, dist, cuts.12=NULL,cuts.13=NULL,cuts.23=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.23=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, conf.int=TRUE, silent=TRUE, precision=10^(-6))
...
if (min( c(1,12,13,123) %in% names(table(sequence)))==0)
        cat("Warning: all possible sequences (1, 12, 13, 123) are not present \n")
...
if (!all(is.na(t2[which(sequence==1 | sequence==13)])))		
	stop("Argument 't2' have to be NA for right-censoring in X=1 or individuals who directly transited from X=1 to X=3")	
...
if (sum(apply(sapply(data.frame(cov.23),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.23),is.na),1,sum)),"individuals with missing values on 'cov.23' will be removed from tha analysis \n")
...
c1<-function(t1, z12, z13, cut12, cut13)
 {return( -H12(t1, z12, cut12) - H13(t1, z13, cut13) ) }
...
c123<-function(t1, t2, z12, z13, z23, cut12, cut13, cut23)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) + log.h23(t2, z23, cut23) - H23(t1, t2, z23, cut23))}

c12<-function(t1, t2, z12, z13, z23, cut12, cut13, cut23)
 {return(log.h12(t1, z12, cut12) - H12(t1, z12, cut12) - H13(t1, z13, cut13) - H23(t1, t2, z23, cut23))}
