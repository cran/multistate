...
pred.mm2 <- function(model, event, times, cov.12=NULL, cov.13=NULL, cov.p=NULL)
{
#check conditions
...
if (model$object!="mm2 (mixture model with two competing events)") 
        stop("'model' have to be the list results of the function 'mm2'")		

if (missing(event)) 
        stop("Argument 'event' is missing with no default")
		
if (!is.numeric(event) | (event!=2 & event!=3)) 
        stop("Argument 'event' must be a numeric value equals to 2 or 3")
...
if (model$covariates[1]!=max(0, dim(cbind(cov.12))[2]))
		stop("The number of covariates related to the time-to-event #2 does not correspond with the 'model'")

if (model$covariates[2]!=max(0, dim(cbind(cov.13))[2]))
		stop("The number of covariates related to the time-to-event #3 does not correspond with the 'model'")
