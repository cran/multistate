...
mm2 <-function(t, sequence, weights=NULL, dist, cuts.12=NULL, cuts.13=NULL, ini.dist.12=NULL, ini.dist.13=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.p=NULL, init.cov.p=NULL, names.p=NULL, init.intercept.p=NULL, conf.int=TRUE, silent=TRUE, precision=10^(-6))
{
cat("\n ******  Mixture model with 2 competing events ****** \n")

#check conditions
if (missing(t)) 
        stop("Argument 't' is missing with no default")
...
if (min(t,na.rm=T)<0)
		stop("Negative values for 't' are not allowed")
		
if (is.na(min(t)))
		cat("Warning: individuals with missing values for 't' will be removed from the analysis \n")
		
if (!is.vector(sequence) | !is.numeric(sequence) | (min(names(table(sequence)) %in% c(1,12,13))==0) )
        stop("Argument 'sequence' must be a numeric vector with values 1, 12, or 13")
		
if (min( c(1,12,13) %in% names(table(sequence)))==0)
        cat("Warning: all sequences (1, 12, and 13) are not present \n ")
        
if (min(length(t), length(sequence)) != max(length(t), length(sequence)))
        stop("Arguments 't' and 'sequence' need to have the same number of rows")
...
if(dist[1]=="PE" & !is.null(cuts.12))
{
 if (max(cuts.12)>=max(t,na.rm=T)) 
 {stop("Arguments 'cuts.12': check internal timepoints or time units (last internal timepoint is greater or equal to the maximum value for 't')")}
}
...
if(dist[2]=="PE" & !is.null(cuts.13))
{
 if (max(cuts.13)>=max(t,na.rm=T)) 
 {stop("Arguments 'cuts.13': check internal timepoints or time units (last internal timepoint is greater or equal to the maximum value for 't')")}
}
...
if (nrow(data.frame(cov.12))!=length(t))
 {stop("Argument 'cov.12' needs to have the same number of rows than 't'")}
...
if (nrow(data.frame(cov.13))!=length(t))
 {stop("Argument 'cov.13' needs to have the same number of rows than 't'")}
...
if (nrow(data.frame(cov.p))!=length(t))
 {stop("Argument 'cov.p' needs to have the same number of rows than 't'")}

if (sum(apply(sapply(data.frame(cov.p),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.p),is.na),1,sum)),"individuals with missing values on 'cov.p' will be removed from the analysis\n")
...
c1<-function(t, z12, z13, zp, cut12, cut13)
 {return( log ( p2(zp) * exp(-H12(t, z12, cut12)) + p3(zp) * exp(-H13(t, z13, cut13)) ) ) }

c12<-function(t, z12, zp, cut12)
 {return(log.p2(zp) + log.h12(t, z12, cut12) - H12(t, z12, cut12)  )}

c13<-function(t, z13, zp, cut13)
 {return(log.p3(zp) + log.h13(t, z13, cut13) - H13(t, z13, cut13)  )}

#missing data
.D <- cbind(t, cov.12, cov.13, cov.p, weights)
.na <- (!is.na(apply(.D, FUN="sum", MARGIN=1)))

#initialization of the parameters
if (is.null(cov.12)) {cov.12.mat <- cbind(rep(0, length(t))); n.12 <- NULL} else { cov.12.mat <- cbind(cov.12); n.12 <- paste("covariate(s) on trans. 12:  num", 1:ncol(data.frame(cov.12))); if(!is.null(names.12)) {n.12 <- names.12} }

if (is.null(cov.13)) {cov.13.mat <- cbind(rep(0, length(t))); n.13 <- NULL} else { cov.13.mat <- cbind(cov.13); n.13 <- paste("covariate(s) on trans. 13:  num", 1:ncol(data.frame(cov.13))); if(!is.null(names.13)) {n.13 <- names.13} }

if (is.null(cov.p)) {cov.p.mat <- cbind(rep(0, length(t))); n.p <- NULL} else { cov.p.mat <- cbind(cov.p); n.p <- paste("covariate(s) associated with P(X=2):  num", 1:ncol(data.frame(cov.p))); if(!is.null(names.p)) {n.p <- names.p} }
...
if (is.null(weights)) {w <- rep(1, length(t))} else {w <- weights }
...
.c1.p <- cov.p.mat[(sequence==1 & .na),]

.w12 <- w[(sequence==12 & .na)]
.t12 <- t[(sequence==12 & .na)]
.c12.12 <- cov.12.mat[(sequence==12 & .na),]
.c12.p <- cov.p.mat[(sequence==12 & .na),]

.w13 <- w[(sequence==13 & .na)]
.t13 <- t[(sequence==13 & .na)]
.c13.13 <- cov.13.mat[(sequence==13 & .na),]
.c13.p <- cov.p.mat[(sequence==13 & .na),]
...
return(list(
object="mm2 (mixture model with two competing events)",
dist=dist,
cuts.12=cuts.12,
cuts.13=cuts.13,
