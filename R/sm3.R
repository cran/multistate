...
sm3 <-function(t1, t2, sequence, weights=NULL, dist, cuts.12=NULL,cuts.13=NULL,cuts.23=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.23=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, conf.int=TRUE, silent=TRUE, precision=10^(-6))
{
...
if (min(t1,na.rm=T)<0)
		stop("Negative values for 't1' are not aollowed")
if (is.na(min(t1)))
...
if (min(t2,na.rm=T)<0)
		stop("Negative values for 't2' wil be removed from the analysis")
...
if (!all(is.na(t2[which(sequence==1 | sequence==13)])))		
		stop("Arguments 't2' have to be NA for right-censored individuals in X=1 or individuals who directly transited from X=1 to X=3")
...
return(list(
object="sm3 (3-state semi-markov model)",
dist=dist,
cuts.12=cuts.12,
cuts.13=cuts.13,
cuts.23=cuts.23,
