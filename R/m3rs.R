...
m3rs <- function(t1, t2, sequence, weights=NULL, dist, cuts.12=NULL,cuts.13=NULL,cuts.23=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.23=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, p.age, p.sex, p.year, p.rate.table, conf.int=TRUE, silent=TRUE, precision=10^(-6))
...
if (min(length(t1),length(t2),length(sequence)) != max(length(t1),length(t2),length(sequence)))
        stop("Arguments 't1', 't2', and 'sequence' need to have the same length")
if (!all(is.na(t2[which(sequence==1 | sequence==13)])))		
	stop("Arguments 't2' should be NA for right-censored individuals in X=1 or individuals who directly transited from X=1 to X=3")
...
if(dist[3]!="PE" & (!is.null(cuts.23)))
 {stop("Arguments 'cuts.23' is only allowed for piecewise exponential distribution (PE for the  thrid argument in 'dist')")} 
...
if(max(p.age)<365.24)
 {cat("Warning: some ages at the baseline are very low (<1 year), check that 'p.age' is in days")} 
if (is.na(min(p.age)))
		cat("Warning: individuals with missing values on 'p.age' will be removed from the analysis \n")
...
if (is.na(min(p.sex)))
		cat("Warning: individuals with missing values on 'p.sex' will be removed from the analysis \n")
...
if (is.na(min(p.year)))
		cat("Warning: individuals with missing values on 'p.year' will be removed from the analysis \n")
...
return(list(
object="m3rs (3-state relative survival markov model with additive risks)",
dist=dist,
cuts.12=cuts.12,
cuts.13=cuts.13,
cuts.23=cuts.23,
