...
sm3ic <-function(t0, t1, t2, sequence, weights=NULL, dist, cuts.12=NULL,cuts.13=NULL,cuts.23=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.23=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, conf.int=TRUE, silent=TRUE, precision=10^(-6), legendre=30, homogeneous=TRUE)
{
...
#check conditions
if (missing(t0)) 
        stop("Argument 't0' is missing with no default")
if (missing(t1)) 
...
if (!is.vector(t0) | !is.numeric(t0))
        stop("Argument 't0' must be a numeric vector")
if (min(t0,na.rm=T)<0)
		stop("Negative values for 't0' are not allowed")
if (is.na(min(t0)))
		cat("Warning: individuals with missing values for 't0' will be removed \n")
...
if (min(length(t0),length(t1),length(t2),length(sequence)) != max(length(t0),length(t1),length(t2),length(sequence)))
        stop("Arguments 't0', 't1', 't2', and 'sequence' need to have the same number of rows")
		
if (!all(is.na(t1[which(sequence==1 | sequence==13)])))	
		stop("Arguments 't1' should be NA for individuals right-censored in X=1 or individuals who directly transited from X=1 to X=3")
if (!all(t0[which(sequence==1| sequence==13)]<= t2[which(sequence==1| sequence==13)],na.rm=T) | !all(t0[which(sequence==12| sequence==123)]<= t1[which(sequence==12| sequence==123)],na.rm=T)  | !all(t1[which(sequence==12| sequence==123)]<= t2[which(sequence==12| sequence==123)],na.rm=T))		
		stop("Arguments 't0','t1','t2': observed times do not respect a	chronological order (t0<=t1<=t2)")
		
if (!all(t0[which(sequence==1| sequence==13)]!= t2[which(sequence==1| sequence==13)],na.rm=T))
		cat("Warning: t0=t2 for some right-censored individuals in X=1 or who directly transited from X=1 to X=3 \n")
		
if (!all(t0[which(sequence==12| sequence==123)]!= t1[which(sequence==12| sequence==123)],na.rm=T))	
		cat("Warning: t0=t1 for some individuals who transited in X=2 \n")	
		
if (!all(t1[which(sequence==12| sequence==123)]!= t2[which(sequence==12| sequence==123)],na.rm=T))		
		cat("Warning: t1=t2 for some individuals who transited in X=2 \n")
...
if (sum(apply(sapply(data.frame(cov.12),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.12),is.na),1,sum)),"individuals with missing values on 'cov.12' will be removed from the analysis \n")
...
if (sum(apply(sapply(data.frame(cov.13),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.13),is.na),1,sum)),"individuals with missing values on 'cov.13' will be removed from the analysis \n")
...
if (sum(apply(sapply(data.frame(cov.23),is.na),1,sum))>0)
		cat("Warning:",sum(apply(sapply(data.frame(cov.23),is.na),1,sum)),"individuals with missing values on 'cov.23' will be removed from the analysis \n")
...
i13 <- function(binf, bsup, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) - H13(m * noeuds[i] + p, y13, cut13)) * h23(bsup - m * noeuds[i] - p, y23, cut23) * exp(-H23(bsup - m * noeuds[i] - p, y23, cut23))
}
return(m*(.temp))
}

c13<-function(d0, d2, z12, z13, z23, cut12, cut13, cut23)
 {return( h13(d2, z13, cut13) * exp(- H12(d2, z12, cut12) - H13(d2, z13, cut13)) + i13(d0, d2, z12, z13, z23, cut12, cut13, cut23) )}

c123 <- function(binf, bsup, tot, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) - H13(m * noeuds[i] + p, y13, cut13)) * h23(tot - m * noeuds[i] - p, y23, cut23) * exp(-H23(tot - m * noeuds[i] - p, y23, cut23))
}
return(m*(.temp))
}

i1 <- function(binf, bsup, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) -H13(m * noeuds[i] + p, y13, cut13) ) * exp(-H23(bsup - m * noeuds[i] - p, y23, cut23))
}
return(m*(.temp))
}

c1<-function(d0, d2, z12, z13, z23, cut12, cut13, cut23)
 {return( exp(- H12(d2, z12, cut12) - H13(d2, z13, cut13)) + i1(d0, d2, z12, z13, z23, cut12, cut13, cut23) )}


c12 <- function(binf, bsup, tot, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) - H13(m * noeuds[i] + p, y13, cut13)) * exp(-H23(tot - m * noeuds[i] - p, y23, cut23))
}
return(m*(.temp))
}
...
#missing data
.D <- cbind(t0, t2, cov.12, cov.13, cov.23, weights)
.na <- (!is.na(apply(.D, FUN="sum", MARGIN=1)))

#initialization of the parameters
if (is.null(cov.12)) {cov.12.mat <- cbind(rep(0, length(t1))); n.12 <- NULL} else { cov.12.mat <- cbind(cov.12); n.12 <- paste("covariate(s) on trans. 12:  num", 1:ncol(data.frame(cov.12))); if(!is.null(names.12)) {n.12 <- names.12} }

if (is.null(cov.13)) {cov.13.mat <- cbind(rep(0, length(t1))); n.13 <- NULL} else { cov.13.mat <- cbind(cov.13); n.13 <- paste("covariate(s) on trans. 13:  num", 1:ncol(data.frame(cov.13))); if(!is.null(names.13)) {n.13 <- names.13} }

if (is.null(cov.23)) {cov.23.mat <- cbind(rep(0, length(t1))); n.23 <- NULL} else { cov.23.mat <- cbind(cov.23); n.23 <- paste("covariate(s) on trans. 23:  num", 1:ncol(data.frame(cov.23))); if(!is.null(names.23)) {n.23 <- names.23} }
...
if (is.null(weights)) {w <- rep(1, length(t1))} else {w <- weights }

#parameters for contributions associated to each transition
.w1 <- w[(sequence==1 & .na)]
.t1.0 <- t0[(sequence==1 & .na)]
.t1.2 <- t2[(sequence==1 & .na)]
.c1.12 <- cov.12.mat[(sequence==1 & .na),]
.c1.13 <- cov.13.mat[(sequence==1 & .na),]
.c1.23 <- cov.23.mat[(sequence==1 & .na),]

.w13 <- w[(sequence==13 & .na)]
.t13.0 <- t0[(sequence==13 & .na)]
.t13.2 <- t2[(sequence==13 & .na)]
.c13.12 <- cov.12.mat[(sequence==13 & .na),]
.c13.13 <- cov.13.mat[(sequence==13 & .na),]
.c13.23 <- cov.23.mat[(sequence==13 & .na),]

.w123 <- w[(sequence==123 & .na)]
.t123.0 <- t0[(sequence==123 & .na)]
.t123.1 <- t1[(sequence==123 & .na)]
.t123.2 <- t2[(sequence==123 & .na)]
...
.t12.0 <- t0[(sequence==12 & .na)]
.t12.1 <- t1[(sequence==12 & .na)]
.t12.2 <- t2[(sequence==12 & .na)]
...
return( -1*(
 sum( .w1 * log(c1(.t1.0, .t1.2, .c1.12, .c1.13, .c1.23, cuts.12, cuts.13, cuts.23)) ) +
 sum( .w13 * log(c13(.t13.0, .t13.2, .c13.12, .c13.13, .c13.23, cuts.12, cuts.13, cuts.23)) ) +
 sum( .w123 * log(c123(.t123.0, .t123.1, .t123.2, .c123.12, .c123.13, .c123.23, cuts.12, cuts.13, cuts.23)) ) + 
 sum( .w12 * log(c12(.t12.0, .t12.1, .t12.2, .c12.12, .c12.13, .c12.23, cuts.12, cuts.13, cuts.23)) ) ) )
...
#contributions to the log-likelihood
i13nh <- function(binf, bsup, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) - H13(m * noeuds[i] + p, y13, cut13)) * h23nh(bsup - m * noeuds[i] - p, y23, cut23, m * noeuds[i] + p) * exp(-H23nh(bsup - m * noeuds[i] - p, y23, cut23, m * noeuds[i] + p))
}
return(m*(.temp))
}

c13nh<-function(d0, d2, z12, z13, z23, cut12, cut13, cut23)
 {return( h13(d2, z13, cut13) * exp(- H12(d2, z12, cut12) - H13(d2, z13, cut13)) + i13nh(d0, d2, z12, z13, z23, cut12, cut13, cut23) )}

c123nh <- function(binf, bsup, tot, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) - H13(m * noeuds[i] + p, y13, cut13)) * h23nh(tot - m * noeuds[i] - p, y23, cut23, m * noeuds[i] + p) * exp(-H23nh(tot - m * noeuds[i] - p, y23, cut23, m * noeuds[i] + p))
}
return(m*(.temp))
}

i1nh <- function(binf, bsup, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) -H13(m * noeuds[i] + p, y13, cut13) ) * exp(-H23nh(bsup - m * noeuds[i] - p, y23, cut23, m * noeuds[i] + p))
}
return(m*(.temp))
}

c1nh<-function(d0, d2, z12, z13, z23, cut12, cut13, cut23)
 {return( exp(- H12(d2, z12, cut12) - H13(d2, z13, cut13)) + i1nh(d0, d2, z12, z13, z23, cut12, cut13, cut23) )}


c12nh <- function(binf, bsup, tot, y12, y13, y23, cut12, cut13, cut23)
{
m <- 0.5*(bsup - binf)
p <- 0.5*(bsup + binf)
.temp <- 0
for(i in 1:legendre)
{
.temp <- .temp + poids[i] * h12(m * noeuds[i] + p, y12, cut12) * exp(-H12(m * noeuds[i] + p, y12, cut12) - H13(m * noeuds[i] + p, y13, cut13)) * exp(-H23nh(tot - m * noeuds[i] - p, y23, cut23, m * noeuds[i] + p))
}
return(m*(.temp))
}
...
return( -1*(
 sum( .w1 * log(c1nh(.t1.0, .t1.2, .c1.12, .c1.13, .c1.23, cuts.12, cuts.13, cuts.23)) ) +
 sum( .w13 * log(c13nh(.t13.0, .t13.2, .c13.12, .c13.13, .c13.23, cuts.12, cuts.13, cuts.23)) ) +
 sum( .w123 * log(c123nh(.t123.0, .t123.1, .t123.2, .c123.12, .c123.13, .c123.23, cuts.12, cuts.13, cuts.23)) ) + 
 sum( .w12 * log(c12nh(.t12.0, .t12.1, .t12.2, .c12.12, .c12.13, .c12.23, cuts.12, cuts.13, cuts.23)) ) ) )
}
